...
async function currentHistoryWithRow() {
  const rows = PedramExcelExporter.configuredRows(current, config);
  let history = await loadHistory();
  rows.forEach(row => { history = mergeHistory(history, row); });
...
function renderPreview() {
  ...
  const rows = PedramExcelExporter.configuredRows(current, config);
  const headers = PedramExcelExporter.outputHeaders(config, rows);
  $('#preview').innerHTML = `<table><thead><tr>${headers.map(header => `<th>${header}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr>${row.values.map(value => `<td>${value || '-'}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
}

function validate() {
  if (!current) return ['ابتدا صفحه را اسکن کنید'];
  const warnings = [];
  const rows = window.PedramExcelExporter?.configuredRows(current, config) || [];
  if (rows.some(row => row.values.some(value => !value))) warnings.push('چند ستون خروجی خالی است');
