(() => {
  ...
  function valueByHeader(row, matchers) {
    ...
    for (const matcher of matchers) {
      ...
      if (found) return cleanAmount(found[1]);
    }
    return '';
  }
  ...
  function invoiceSummaryRow(data) {
    return invoiceDefaultRows(data)[0];
  }
  ...
  function configuredRows(data, config) {
    const enabled = (config?.fields || []).filter(field => field.enabled !== false);
    return enabled.length ? [configuredRow(data, config)] : invoiceDefaultRows(data);
  }

  function makeWorkbook(data, config = {}, history = []) {
    const currentRows = configuredRows(data, config);
    const sheets = [{ name: 'خروجی صورتحساب‌ها', rows: summaryRows(history.length ? history : currentRows, config) }];
  ...
  window.PedramExcelExporter = { makeWorkbook, invoiceSummaryRow, configuredRow, configuredRows, outputHeaders, DEFAULT_HEADERS };
})();
